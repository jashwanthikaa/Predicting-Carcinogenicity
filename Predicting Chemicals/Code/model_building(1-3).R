...
best_coords <- coords(roc_obj, "best", ret = c("threshold", "specificity", "sensitivity", "accuracy"))
best_thresh <- best_coords$threshold  # or as.numeric(best_coords["threshold"])
cat("\nOptimal threshold based on ROC:", best_thresh, "\n")

# Final predicted classes using optimized threshold
xgb_pred <- ifelse(xgb_pred_prob > as.numeric(best_thresh), 1, 0)

# Confusion matrix
xgb_cm <- confusionMatrix(factor(xgb_pred), factor(y_test), positive = "1")
cat("\nConfusion Matrix:\n")
print(xgb_cm)
...
write.csv(results, "xgb_model_predictions(1-3).csv", row.names = FALSE)
